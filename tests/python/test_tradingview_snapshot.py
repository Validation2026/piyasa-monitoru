...
def test_phase_one_manifest_has_critical_forex_and_crypto():
    instruments = load_instruments()
    by_id = {item.series_id: item for item in instruments}

    assert len(instruments) == 52
    assert by_id["BZ=F"].override_mode == "persistent"
    assert by_id["USDTRY=X"].files == ("currencies.json",)
    assert by_id["BTC-USD"].files == ("crypto.json",)
    assert by_id["BTC-USD"].override_mode == "fresh_only"


def test_fresh_only_snapshot_expires_but_critical_snapshot_does_not():
    now = datetime(2026, 7, 29, 0, 0, tzinfo=timezone.utc)
    fresh_only = {
        "captured_at": "2026-07-27T04:00:00Z",
        "override_mode": "fresh_only",
        "max_age_hours": 36,
    }
    persistent = {**fresh_only, "override_mode": "persistent"}

    assert snapshot_is_fresh(fresh_only, now) is False
    assert snapshot_is_fresh(persistent, now) is True


def test_expired_snapshot_does_not_replace_yahoo():
    entry = {"id": "EURUSD=X", "current": 1.18}
    snapshots = {
        "EURUSD=X": {
            "price": 1.2,
            "captured_at": "2026-07-27T04:00:00Z",
            "resolved_url": "https://www.tradingview.com/symbols/EURUSD/",
            "override_mode": "fresh_only",
            "max_age_hours": 36,
        }
    }

    applied = overlay_series_entry(
        entry, snapshots, datetime(2026, 7, 29, 0, 0, tzinfo=timezone.utc)
    )

    assert applied is False
    assert entry["current"] == 1.18
