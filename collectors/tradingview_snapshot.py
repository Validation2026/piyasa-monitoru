...
SYMBOLS_FILE = Path(__file__).with_name("tradingview_symbols.json")
...
class Instrument:
    ...
    url: str
    files: tuple[str, ...]
    override_mode: str = "fresh_only"
    max_age_hours: int = 36


def load_instruments(path: Path = SYMBOLS_FILE) -> tuple[Instrument, ...]:
    """Load and validate the staged Yahoo-to-TradingView symbol manifest."""
    ...
    for raw in payload.get("instruments", []):
        ...
        if series_id in seen:
            raise ValueError(f"Tekrarlanan TradingView eşlemesi: {series_id}")
        seen.add(series_id)
        mode = raw.get("override_mode", "fresh_only")
        if mode not in {"persistent", "fresh_only"}:
            raise ValueError(f"Geçersiz override_mode ({series_id}): {mode}")
        instruments.append(
            Instrument(
                series_id=series_id,
                name=raw["name"],
...
def capture(page: Any, instrument: Instrument, artifact_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "override_mode": instrument.override_mode,
            "max_age_hours": instrument.max_age_hours,
        }
    except Exception:
...
def snapshot_is_fresh(snapshot: dict[str, Any], now: datetime | None = None) -> bool:
    """Persistent critical quotes never expire; staged quotes require freshness."""
    if snapshot.get("override_mode", "persistent") == "persistent":
        return True
    try:
        ...
        return 0 <= age_hours <= float(snapshot.get("max_age_hours", 36))
    except (KeyError, TypeError, ValueError):
        return False


def overlay_series_entry(
    entry: dict[str, Any],
    ...
    if not snapshot or not snapshot_is_fresh(snapshot, now):
        return False
